...
def set():
    ...
    try:
        ...
        v, c = embd_mdl.encode([
            ...
            settings.EMBEDDING_DOCUMENT_PREFIX + req["content_with_weight"] if not d.get("question_kwd") else "\n".join(d["question_kwd"])]
        )
...
def create():
    ...
    try:
        ...
        v, c = embd_mdl.encode([
            settings.EMBEDDING_DOCUMENT_PREFIX + doc.name,
            settings.EMBEDDING_DOCUMENT_PREFIX + req["content_with_weight"] if not d["question_kwd"] else "\n".join(d["question_kwd"])
        ])
        v = 0.1 * v[0] + 0.9 * v[1]
        d["q_%d_vec" % len(v)] = v.tolist()
